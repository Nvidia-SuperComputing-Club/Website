 ...
 * A step may split its options into `groups`, each with its own heading.
 */
export default function ChoiceGrid({ step, value, onChange, onAdvance, reducedMotion }) {
  ...
  const groups = step.groups || [{ id: 'all', options: step.options }];
  const flatOptions = groups.flatMap((g) => g.options);
  const knownValues = flatOptions.map((o) => o.value);
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      ...
      if (!Number.isInteger(n) || n < 1 || n > Math.min(flatOptions.length, 9)) return;
      e.preventDefault();
      choose(flatOptions[n - 1]);
    };
  ...
  const renderOption = (option, indexInGroup) => {
    const Icon = option.icon;
    const active = isSelected(option);
    const dimmed = full && !active;
    optionNumber += 1;
    const hotkey = optionNumber <= 9 ? optionNumber : null;
    ...
    return (
      <motion.button
        ...
        transition={{ delay: reducedMotion ? 0 : indexInGroup * 0.035, duration: 0.25 }}
        whileTap={reducedMotion ? undefined : { scale: 0.97 }}
        className={`group relative flex min-h-[56px] items-center gap-3 rounded-2xl border p-3.5 text-left transition-colors duration-200 ${
          active
  ...
  return (
    <div>
      <div className="space-y-5">
        {groups.map((group) => {
          const nudge = group.nudge?.(multi ? selected : [selected].filter(Boolean));
          return (
            <section key={group.id} aria-label={group.label}>
              {group.label && (
                <header className="mb-2.5">
                  <div className="flex items-center gap-2">
                    <h3 className="font-mono text-[11px] uppercase tracking-[0.16em] text-gray-300">
                      {group.label}
                    </h3>
                    {group.badge && (
                      <span className="rounded-full border border-nvidia/30 bg-nvidia/10 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider text-nvidia">
                        {group.badge}
                      </span>
                    )}
                  </div>
              ...
              <AnimatePresence initial={false}>
                {nudge && (
                  <motion.p
                    ...
                    className="mb-2.5 flex gap-2 overflow-hidden rounded-2xl border border-nvidia/25 bg-nvidia/[0.07] p-3 text-[12px] leading-relaxed text-gray-200"
                  >
                    <Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0 text-nvidia" />
                    <span>{nudge}</span>
                  </motion.p>
                )}
              </AnimatePresence>
              ...
              <div
                ...
                aria-label={group.label || step.question}
              >
      ...
      {multi && (
        <p className="mt-3 text-center font-mono text-[11px] uppercase tracking-wider text-gray-500">
